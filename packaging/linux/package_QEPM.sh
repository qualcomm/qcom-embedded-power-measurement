...
if [ ! -d "$EXAMPLES_SRC_DIR" ]; then
    echo "ERROR: Missing examples directory: $EXAMPLES_SRC_DIR" >&2
    missing=1
fi
...
Depends: bash, coreutils, libxcb-cursor0, libxcb-icccm4, libxcb-util1, libxcb-image0, libxcb-keysyms1, libxcb-render-util0, libusb-0.1-4
...
if ! ldconfig -p 2>/dev/null | grep -q "libxcb-cursor.so.0"; then
    ...
    echo "[QEPM] libxcb-cursor0 not found. Installing..." \
        >> "\$LOG_FILE"
    ...
    if ldconfig -p 2>/dev/null | grep -q "libxcb-cursor.so.0"; then
        ...
        echo "[QEPM] Successfully installed libxcb-cursor0." \
            >> "\$LOG_FILE"

    else

        echo "[QEPM] WARNING: Failed to install libxcb-cursor0." \
            >> "\$LOG_FILE"
...
echo "[QEPM] Installation complete." >> "\$LOG_FILE"
...
LOG_FILE="/opt/qcom/QEPM/qepm_install.log"

if [ -f "$LOG_FILE" ]; then
    echo "" >> "$LOG_FILE" 2>&1
    echo "==============================================================" >> "$LOG_FILE" 2>&1
    echo "[QEPM] Uninstall started: $(date)" >> "$LOG_FILE" 2>&1
    echo "==============================================================" >> "$LOG_FILE" 2>&1
fi
