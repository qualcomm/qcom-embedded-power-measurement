...
int main(int argc, char* argv[])
{
	...
	result = GetAlpacaVersion(temp, sizeof(temp));
	if (result == NO_EPM_ERROR)
	...
	if (result == NO_EPM_ERROR)
	{
		...
		if (deviceCount > 0)
		{
			...
			if (epmHandle != kBadHandle)
			{
				...
				{
					// absolute path
					//if (SetRuntimeConfig(epmHandle, "C:\\Users\\msimpson\\Documents\\Alpaca\\Runtime Configurations\\SM8450_Battery.rcnf") == NO_EPM_ERROR)
					if (SetRuntimeConfig(epmHandle, "C:\\Alpaca\\Examples\\data\\SM8450_Battery.rcnf") == NO_EPM_ERROR) // from preferences
					{
						int channelCount;
