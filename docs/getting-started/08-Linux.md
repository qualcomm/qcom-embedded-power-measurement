...
## Introduction
QEPM on Linux is a combination of software and circuitry which integrates debug, power measurement and
test automation capabilities into a single board. The device to be tested is connected to the host using a USB cable.

## Supported Linux distributions
QEPM on Linux is supported for Ubuntu 22.04 and above. The latest version of QEPM may not work correctly
on older versions of Linux.

## Install using QPM
QEPM on Linux can be downloaded using [Qualcomm Package Manager](https://qpm.qualcomm.com) or installed
using the qpm-cli by issuing the command `qpm-cli --install QEPM`

> It is important to execute the `postinstall.sh` script present at ~/QEPM/postinstall.sh. The postinstall
script contains rules that help the host machine recognize the connected TAC devices.
...
using `cat postinstall.sh` and manually executing the commands on the terminal.

## Necessary libraries and versions
QEPM requires the following libraries on the Linux machine to work optimally. If applications shipped with
QEPM fail to start, use the install command column to install the required Linux packages. It is always a
...
Once installed, the application binaries are present at **/opt/qcom/QEPM/bin**. The offline documentation is
...
- TACDev: Python library for QEPM automation control
- EPMDev: Python library for QEPM power measurements. You will need a PSOC debug board
- UDASDev: Python library for QEPM interpreting and parsing EPM data. A physical connection to the debug board
is not required in order to work with this library

## Troubleshooting
In case you are facing issues with the debug board and QEPM, you can perform the following basic triage to decide
if you're running into a setup issue. If the triage steps below don't resolve the issue, open a GitHub issue at
https://github.com/qualcomm/qcom-embedded-power-measurement/issues.
...
## FAQ
**Q. Why are we not supporting older flavours of Ubuntu than 22.04?**

A. Older versions of Ubuntu do not have the features needed for modern debug boards.

**Q. Why is the versioning of QEPM on Linux not consistent / incremental?**
...
If you have connected an FTDI-based debug board, you should see the following FTDI device.
...
`/etc/udev/rules.d`. Presence of conflicting rules can hinder proper access to the TAC device. QEPM will only
add the following udev rule:
1. 99-FTDI-USB.rules

If you do not find the above rules in the directory, please execute `postinstall.sh` from `/opt/qcom/QEPM/util`.

If you still cannot connect to the TAC device through TAC, make sure the user is part of the **dialout** group. You can check
group access for the user using `groups _<username>_`
