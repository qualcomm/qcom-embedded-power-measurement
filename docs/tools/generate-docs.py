...
"""
Builds the QEPM HTML documentation from the markdown sources in
...
def get_qepm_version() -> str:
    """
    Extracts the QEPM_VERSION string from version.h so the built documentation
    always matches the version of the software it documents.
    """
    match = re.search(r'QEPM_VERSION\s+"([^"]+)"', VERSION_FILE.read_text(encoding="utf-8"))
    if match is None:
        raise RuntimeError(f"Could not find QEPM_VERSION in {VERSION_FILE}")
...
def main() -> None:
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        Sphinx(
            ...
            buildername="html",
            confoverrides={"version": get_qepm_version(), "release": get_qepm_version()},
        ).build()
