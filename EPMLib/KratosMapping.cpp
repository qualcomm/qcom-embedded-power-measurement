...
static ChannelMap kAlpacaMapping
{
...
{
	return std::get<kMarkerTupleIndex>(kAlpacaMapping[marker]);
}
...
) const
{
	...
	{
		auto kme = kAlpacaMapping.find(rcmChannel);
		if (kme != kAlpacaMapping.end())
		{
...
) const
{
	...
	if (_spmVersion == eLegacyVersion)
	{
		...
		if (kme != kAlpacaMapping.end())
		{
			result = std::get<kAlpacaTupleIndex>(kme.value());
		}
	}
...
) const
{
	...
	if (_spmVersion == eLegacyVersion)
	{
		...
		auto kme = kAlpacaMapping.begin();
		while (kme != kAlpacaMapping.end())
		{
