...
# Note: This file is part of the QEPM / QEPM installation and cannot be distributed separately.
...
class _SetupUDAS:
    ...
    def setupSharedLibraryPath(self) -> None:
        ...
        if currentPlatform.startswith("linux") and pythonIsDebugging:
            ...
            # packaged distribution that bundles UDAS under its own product
            ...
            if not self.__udasLibraryPath.exists():
                pathLibrary = self._findInstalledLibraryOnPath("UDASDev.dll")
                if pathLibrary is not None:
                    self.__udasLibraryPath = pathLibrary
...
if udasLibraryPath is not None:
    try:
        ...
        if not udasLibrary:
            ...
            __QEPMVersionFunc = udasLibrary.GetQEPMVersion
...
class UDASDevice:
    ...
    def OpenResults(self, pathToResultsFolder) -> None:
        ...
        :param pathToResultsFolder: The path to the captured EPM data. Usually located at: C:/Users/<username>/Documents/QEPM/Results
        """
...
def QEPMVersion() -> str:
    """
    Returns the version of QEPM
    """
    buf = create_string_buffer(1024)
    __QEPMVersionFunc(buf, sizeof(buf))
    QEPMVersion = buf.value.decode("utf-8")
    return QEPMVersion
...
def main() -> None:
    ...
    QEPMVersion = QEPMVersion()
    print(f"QEPM Version: {QEPMVersion}")
    ...
    epmResultsDirectory = "\%userprofile\%\\Documents\\QEPM\\Results\\CRD8380X2_2024_23_12_12_36_02\\CRD8380X2_2024_23_12_12_36_02.prn"
    udasDev = UDASDevice()
