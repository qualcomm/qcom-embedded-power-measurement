...
if epmLibraryPath is not None:
    try:
        ...
        logger.error(
            f"""Could not locate the EPMDev.dll. Please update QEPM.
        Error: {error}"""
        )
    ...
    try:
        ...
        __QEPMVersionFunc = epmLibrary.GetQEPMVersion
        ...
        logger.error(f"""Could not load methods from EPMDev. Please update QEPM
        and run the setup.bat at C:/ProgramData/Qualcomm/QEPM/Python/XPlatform.
        Error: {error}""")
        exit()
else:
...
def QEPMVersion() -> str:
    """
    Returns the version of QEPM as string.
    """
    buf = create_string_buffer(1024)
...
def main() -> None:
    ...
    """
    print("QEPM Version: ", QEPMVersion())
        ...
        if epmDevice is not None:
            ...
            if epmDevice.Open():
                try:
                    ...
                    epmDevice.SetRuntimeConfig("C:\\QEPM\\Examples\\data\\SM8450_Battery.rcnf")
