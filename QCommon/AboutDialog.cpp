...
)
{
	...
	if (appCore->isInternalBuild() == false)
		workingCopy.replace(QByteArray("Alpaca"), QByteArray("QEPM"));
...
)
{
	...
	AppCore* appCore = AppCore::getAppCore();
	if (appCore->isInternalBuild() == false)
		appTitle.replace(QByteArray("Alpaca"), QByteArray("QEPM"));
...
void AboutDialog::keyPressEvent
(
	QKeyEvent* event
)
{
	if (event->key() == Qt::Key_A)
	{
		...
		{
			_ui->_aboutLine2->setText(aboutText());
		}
	}
...
void AboutDialog::keyReleaseEvent
(
	QKeyEvent* event
)
{
	_ui->_aboutLine2->clear();
