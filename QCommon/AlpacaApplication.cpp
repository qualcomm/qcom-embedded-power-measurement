...
bool AlpacaApplication::initialize(PreferencesBase *preferencesBase)
{
	...
	if (isInternalBuild())
	{
		_dateStale = _appCore->daysSinceInstall() > 90;
		if (_dateStale)
		{
			QByteArray message("Date is 90 days past build date! Upgrade the app from QPM\n");
			_appCore->writeToAppLog(message);
			postEvent(this, new DateCheckFailEvent(message));
		}

		if (_appCore->checkLicense(kProductID, kCoreFeature) == false)
		{
			if (_appCore->isLicenseManagerValid() == true)
			{
				AppCore::writeToApplicationLogLine(kLicenseCheckFailed.decode());
				postEvent(this, new LicenseFailEvent(kLicenseCheckFailed.decode()));
			}
		}
		else
		{
			...
			AppCore::writeToApplicationLogLine("License Good");
