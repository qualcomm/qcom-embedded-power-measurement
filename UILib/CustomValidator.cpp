	...
	QItemDelegate(parent)
{
	...
	case eNumericOnlyValidator:
		...
		break;
	case eFloatingPointValidator:
		setFloatingPointValidation();
		break;
...
void CustomValidator::setFloatingPointValidation()
{
	_re.setPattern("[-]?([0-9]+([.][0-9]*)?|[.][0-9]+)");
	_tooltip = "Floating point values are allowed";
}
...
void CustomValidator::setPlatformIdLimit(QWidget *parent)
{
	if (parent != Q_NULLPTR)
	{
		...
		if (lew != Q_NULLPTR)
		{
			if (_debugBoardType == ePSOC)
			{
				_re.setPattern("^(?:1?[0-9]{1,2}|2[0-4][0-9]|25[0-5])$");
				_tooltip = "Platform id 255 is dedicated for experiments. Programming with platform id > 255 will crash the debug board";

				QRegularExpressionValidator* validator = new QRegularExpressionValidator(_re, parent);
				lew->setValidator(validator);
				lew->setToolTip(_tooltip);
			}
			else if (_debugBoardType == eFTDI)
			{
