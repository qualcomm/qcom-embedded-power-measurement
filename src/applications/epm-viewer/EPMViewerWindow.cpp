	...
	_recentPowerRunFiles(kAppName, "AVRunConfigRecents")
{
	...
	connect(_voltageChart, &QEPMChart::zoomRectChanged, [this] (const QRectF& zoomRect)
	{
	...
	connect(_currentChart, &QEPMChart::zoomRectChanged, [this] (const QRectF& zoomRect)
	{
	...
	connect(_currentChart, &QEPMChart::t0Changed, this, &EPMViewerWindow::onT0Update);
	connect(_voltageChart, &QEPMChart::t0Changed, this, &EPMViewerWindow::onT0Update);
	connect(_powerChart, &QEPMChart::t0Changed, this, &EPMViewerWindow::onT0Update);

	connect(_currentChart, &QEPMChart::t1Changed, this, &EPMViewerWindow::onT1Update);
	connect(_voltageChart, &QEPMChart::t1Changed, this, &EPMViewerWindow::onT1Update);
	connect(_powerChart, &QEPMChart::t1Changed, this, &EPMViewerWindow::onT1Update);
...
void EPMViewerWindow::onAboutTriggered()
{
	...
	aboutDialog.setTitle("<html><head/><body><p><span style=\" font-size:12pt; font-weight:600;\">QEPM Viewer</span></p></body></html>");
	...
	QPixmap pixMap = QPixmap(QString::fromUtf8(":/QEPMViewer_BS.png"));
...
void EPMViewerWindow::onSubmitBugReportTriggered()
{
	...
	arguments << "application:QEPM_Viewer";
...
)
{
	...
	{
		EPMViewerWindow* QEPMViewer = EPMViewerApplication::createViewerWindow();
		result = QEPMViewer->openAndReadPowerProject(powerRunFileName);
	}
	else
...
{
	QEPMChart* QEPMChart(Q_NULLPTR);

	switch (udasBlock->_waveFormType)
	{
	case eWaveFormCurrent:
		QEPMChart = _currentChart;
		break;

	case eWaveFormVoltage:
		QEPMChart = _voltageChart;
		break;

	case eWaveFormPower:
		QEPMChart = _powerChart;
		break;
	...
	if (QEPMChart != Q_NULLPTR)
	{
		QEPMChart->addLineSeries(udasBlock);
	}
}
