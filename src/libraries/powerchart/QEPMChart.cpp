...
(
	...
	QWidget(parent),
	_ui(new Ui_QEPMChartClass),
	_plotGrid(new QwtPlotGrid),
	_cursorLines(new QEPMCursorLines(this))
{
	_ui->setupUi(this);
...
)
{
	...
	QEPMDataSeries QEPMDataSeries = _lineSeriesModel.getDataSeries(seriesHash);

	if (QEPMDataSeries.isNull() == false)
	{
		BasicColor newColor = udasBlock->channelColor();
		QEPMDataSeries->setPen(QColor(newColor.red(), newColor.green(), newColor.blue()), .5);

		replot();
	}
}

uint QEPMChart::addLineSeries
(
	UDASBlock udasBlock
)
{
	QEPMDataSeries QEPMSeries = QEPMDataSeries(new _QEPMDataSeries(udasBlock));

	QEPMSeries->setData(QEPMSeries.data());
	BasicColor seriesColor = QEPMSeries->seriesColor();
	QEPMSeries->setPen(QColor(seriesColor.red(), seriesColor.green(), seriesColor.blue()), .5);
	QEPMSeries->setRenderHint( QwtPlotItem::RenderAntialiased, true );
	QEPMSeries->setVisible(false);
	QEPMSeries->attach(_ui->_plot);

	_lineSeriesModel.addDataSeries(QEPMSeries);

	_active = true;

	return QEPMSeries->seriesHash();
}

void QEPMChart::setGraphBackground(const QColor &backgroundColor)
{
	_ui->_plot->setCanvasBackground(backgroundColor);
}

void QEPMChart::setXTitle(const QString &title)
{
	_ui->_xLabel->setText(title);
}

void QEPMChart::setYTitle(const QString &title)
{
	_ui->_yLabel->setText(title);
}

void QEPMChart::setXAxis(int major, int minor)
{
	_ui->_plot->setAxisMaxMajor(QwtPlot::xBottom, major);
	_ui->_plot->setAxisMaxMinor(QwtPlot::xBottom, minor);
}

void QEPMChart::setXRange(qreal min, qreal max)
{
	_ui->_plot->setAxisScale(QwtPlot::xBottom, min, max);
}

void QEPMChart::setYAxis(int major, int minor)
{
	_ui->_plot->setAxisMaxMajor(QwtPlot::yLeft, major);
	_ui->_plot->setAxisMaxMinor(QwtPlot::yLeft, minor);
}

void QEPMChart::setYRange(qreal min, qreal max)
{
...
void QEPMChart::setTitleColor(const QColor &color)
{
	QwtText axisTitleText;
...
void QEPMChart::setCursorLines(qreal min, qreal max)
{
	_cursorLines->setLeftBoundary(min);
	_cursorLines->setRightBoundary(max);
}

void QEPMChart::setZoomBase(const QRectF &zoomBase)
{
	_okayZoomer->setZoomBase(zoomBase);
}

void QEPMChart::setXZoom(qreal left, qreal right)
{
...
void QEPMChart::emitBandingPosition()
{
	switch (_cursorLines->bandingLine())
	{
	case QEPMCursorLines::eLeft:
		emit t0Changed(_cursorLines->left());
		break;

	case QEPMCursorLines::eRight:
		emit t1Changed(_cursorLines->right());
		break;

	case QEPMCursorLines::eNone:
		break;
	}
}

void QEPMChart::emitBandingComplete()
{
	switch (_cursorLines->bandingLine())
	{
	case QEPMCursorLines::eLeft:
