...
const QString kRegexPattern(QStringLiteral("(Manufacturer Name:\\s*FTDI).*?(Driver Version:\\s*\\S*\\s(\\d{1,2}\\.\\d{1,2}\\.\\d{1,2}\\.\\d{1,2}))"));
const int kRegexVersionGroup(3);
#endif
...
{
#ifdef Q_OS_WIN
	_expression.setPattern(kRegexPattern);
	_expression.setPatternOptions(QRegularExpression::DotMatchesEverythingOption);
#endif
}
...
{
    QString driverVer(kDriverNotFound);

#ifdef Q_OS_WIN
	QProcess process;

	process.start("cmd", QStringList() << "/c" << "pnputil /enum-devices /class USB /drivers");
	process.waitForFinished();

	QString output = process.readAllStandardOutput();

	QRegularExpressionMatch match = _expression.match(output);
	if (match.hasMatch())
		driverVer = match.captured(kRegexVersionGroup).trimmed();
#endif
