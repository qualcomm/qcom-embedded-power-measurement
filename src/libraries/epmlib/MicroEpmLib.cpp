...
static DWORD WINAPI MicroEpmGetDataThread(LPVOID pCtxt)
{
    ...
    while (hDevice->bAcquiring) {
        ...
        if (uNumOverflow != 0) {
            ...
            // TODO
            printf("ERROR: QEPM BUFFER OVERFLOW OCCURRED\n");
...
MICRO_EPM_API MicroEpmErrorType MicroEpmTacCommand(MicroEpmHandle *hDevice, const uint8_t *pCommandStr,uint8_t cmd_size,uint8_t *pRsp,uint8_t *pRspSize,uint8_t RspBufSize)
{
    ...
    if ((hDevice->version.EpmID != MICRO_EPM_BOARD_ID_QEPM) &&
        (hDevice->version.EpmID != MICRO_EPM_BOARD_ID_QEPM_V2) &&
        (hDevice->version.EpmID != MICRO_EPM_BOARD_ID_QEPM_V3) &&
        (hDevice->version.EpmID != MICRO_EPM_BOARD_ID_QEPM_V3P1) &&
