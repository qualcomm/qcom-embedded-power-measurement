...
_EPMDevice* device3Alloctor()
{
	return new _EPMDevice3;
}
...
void _EPMDevice3::updateDeviceList()
{
	try
	{
		_WinBaseDevice::updateDeviceList(MICRO_EPM_3_DEVICE_INTERFACE, device3Alloctor);
	}
	catch (EPMException error)
	{
		AppCore::writeToApplicationLog(QString("_EPMDevice3::updateDeviceList EPM Error %1\n").arg(error.what()));
	}
}
...
)
{
	...
	try
	{
		...
		{
			uMaxArraySampleSize = min(_version._maxSamplesPerPacket, channelDataSize);
